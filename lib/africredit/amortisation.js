 ...
 * @returns {Schedule}
 */
export function buildSchedule({ principalCentimes, annualRatePct, termMonths, firstDueDate }) {
  ...
  for (let n = 1; n <= termMonths; n += 1) {
    ...
    const interest = periodicRate === 0
      ? 0n
      : BigInt(Math.round(Number(balance) * periodicRate));
