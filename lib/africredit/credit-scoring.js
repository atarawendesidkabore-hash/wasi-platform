 ...
 * @property {number} [collateralFullScoreXof]  value earning a full 100 on the
 *   collateral factor. Defaults to COLLATERAL_FULL_SCORE_XOF (50 000 000), which
 *   is calibrated for SME/corporate lending. Microfinance callers should pass the
 *   loan principal instead, turning the factor into a collateral COVERAGE ratio -
 *   with the corporate anchor, a 450 000 XOF village guarantee scores 0,9/100 and
 *   the whole 10 % weight is dead.
...
export function calculateCreditScore(input) {
  ...
  const collateralAnchor = Number(input.collateralFullScoreXof) > 0
    ? Number(input.collateralFullScoreXof)
    : COLLATERAL_FULL_SCORE_XOF;
  const collateralScore = Math.min(
    SCORE_MAX,
    (input.collateralValue * SCORE_MAX) / collateralAnchor
  );
